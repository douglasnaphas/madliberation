...
const getPath = [
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      logger.log(
        `getPath: saved correctPwHash starting ${res.locals.correctPwHash.substring(
          0,
          3
        )}`
      );
      res.locals.path = response.Item.path;
      logger.log(`getPath: saved path ${res.locals.path}`);
      return next();
    } catch (error) {
  ...
  (req, res) => {
    ...
    return res.send({ path: res.locals.path });
  },
];
