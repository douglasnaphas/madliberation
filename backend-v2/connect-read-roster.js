...
exports.handler = async function (event, context, callback) {
  ...
  const putConnectionIdCommand = new PutCommand({
    ...
    Item: {
      ...
      GSI1SK: schema.READ_ROSTER_SOCKET_CONNECTION,
    },
  });
  ...
  if (!putConnectionIdResponse) {
    console.log("failed to save read roster connection id", putConnectionIdResponse);
    return { statusCode: 500, body: "failed to save read roster connection id" };
  }
  console.log(`saved read roster connection id ${event.requestContext.connectionId}`);
  return {
    statusCode: 200,
    body: "Connected",
  };
